package com.kbsriram.drivethrough.android.util;

public interface IConstants
{
    public final static int REQUEST_PLAY_INSTALLED = 50;
    public final static int REQUEST_SELECT_ACCOUNT = 51;
    public final static int REQUEST_USER_AUTH = 52;

    public final static String PREF_DEVICE_PASS = "pref_device_pass";
    public final static String PREF_ONLY_WIFI = "pref_only_wifi";
    public final static String PREF_SELECTED_ACCOUNT = "pref_selected_account";

    public final static String KEY_LAST_RESULT = "last_result";
    public final static String KEY_LAST_KEY_REFRESH = "last_key_refresh";
}
