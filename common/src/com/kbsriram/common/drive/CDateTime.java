...
/**
 * Immutable representation of a date with an optional time and an
 * optional time zone based on <a
 ...
 * @author Yaniv Inbar
 */
public final class CDateTime implements Serializable {
  ...
  /**
   * Instantiates {@link CDateTime} from a {@link Date} and {@link TimeZone}.
   *
   * @param date date and time
   * @param zone time zone; if {@code null}, it is interpreted as {@code TimeZone.getDefault()}.
   */
  public CDateTime(Date date, TimeZone zone) {
    this(false, date.getTime(), zone == null ? null : zone.getOffset(date.getTime()) / 60000);
  }

  /**
   * Instantiates {@link CDateTime} from the number of milliseconds since the Unix epoch.
   *
   * <p>
   ...
   */
  public CDateTime(long value) {
    this(false, value, null);
  }

  /**
   * Instantiates {@link CDateTime} from a {@link Date}.
   *
   * <p>
   * The time zone is interpreted as {@code TimeZone.getDefault()}, which may vary with
   * implementation.
   ...
   */
  public CDateTime(Date value) {
    this(value.getTime());
  }

  /**
   * Instantiates {@link CDateTime} from the number of milliseconds since the Unix epoch, and a shift
   * from UTC in minutes.
   *
   * @param value number of milliseconds since the Unix epoch (January 1, 1970, 00:00:00 GMT)
   * @param tzShift time zone, represented by the number of minutes off of UTC.
   */
  public CDateTime(long value, int tzShift) {
    this(false, value, tzShift);
  }

  /**
   * Instantiates {@link CDateTime}, which may represent a date-only value, from the number of
   ...
   */
  public CDateTime(boolean dateOnly, long value, Integer tzShift) {
  ...
  /**
   * Instantiates {@link CDateTime} from an <a href='http://tools.ietf.org/html/rfc3339'>RFC 3339</a>
   * date/time value.
   *
  ...
  public CDateTime(String value) {
    ...
    // of parseRfc3339 can then do "return new CDateTime(str);".
    CDateTime dateTime = parseRfc3339(value);
   ...
   * </p>
   */
  public static CDateTime parseRfc3339(String str) throws NumberFormatException {
    try {
      ...
      return new CDateTime(dateOnly, value, tzShiftInteger);
